    ...
    def train(self, train_dataloader, val_dataloader):
        try:
            for epoch in tqdm(range(self.epoch_num)):
                ...
                predicted_samples, target_samples = self.model.forward(batch)
                source_sentences = batch[0].cpu().numpy()
                source_sentences = [self.model.tokenizer.decode(x, skip_special_tokens=True, clean_up_tokenization_spaces=True) for x in source_sentences]
                bleu_score, actual_sentences, predicted_sentences = self.model.eval_bleu(predicted_samples, target_samples)
